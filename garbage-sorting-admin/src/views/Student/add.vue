    ...
    <el-card class="box-card">
      <el-form ref="addStuForm" :model="addForm" label-width="80px" :rules="addFormRules">
        ...
        <el-form-item label="楼层：">
          <el-select v-model="addForm.stu_floor" placeholder="请选择楼层" clearable>
            <el-option
        ...
        <el-form-item label="寝室号：">
          <el-input v-model="addForm.stu_room"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" class="btn" @click="addStu">立即创建</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  ...
  methods: {
    addStu () {
      this.$refs.addStuForm.validate(async valid => {
        // 前端验证为false 阻止请求
        if (!valid) return
        const { data: res } = await this.$http.post('http://localhost:8888/admin/addStu', this.addForm)
        if (res.meta.status !== 200) return this.$message.error('新增学生信息失败')
