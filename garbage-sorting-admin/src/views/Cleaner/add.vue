<template>
   <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>添加清洁工</el-breadcrumb-item>
    </el-breadcrumb>

    <!-- 卡片视图区域 -->
    <el-card class="box-card">
      <el-form ref="addCleanerForm" :model="addForm" label-width="80px" :rules="addFormRules">
        ...
        <el-form-item>
          <el-button type="primary" class="btn" @click="addCleaner">立即创建</el-button>
        </el-form-item>
      </el-form>
    </el-card>
...
export default {
  ...
  methods: {
    addCleaner () {
      this.$refs.addCleanerForm.validate(async valid => {
        // 前端验证为false 阻止请求
        if (!valid) return
        const { data: res } = await this.$http.post('http://localhost:8888/admin/addCleaner', this.addForm)
        console.log(res)
        if (res.meta.status !== 200) return this.$message.error('新增清洁工信息失败')
        this.$message.success('新增清洁工信息成功')
