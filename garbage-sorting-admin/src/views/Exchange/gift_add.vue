   ...
   <div>
    ...
    <el-card class="box-card">
      <el-form ref="addGiftForm" :model="addForm" label-width="120px" :rules="addFormRules">
        ...
        <el-form-item>
          <el-button type="primary" class="btn" @click="addGift">立即创建</el-button>
        </el-form-item>
      </el-form>
    </el-card>
   </div>
  ...
  methods: {
    addGift () {
      this.$refs.addGiftForm.validate(async valid => {
        ...
        const { data: res } = await this.$http.post('http://localhost:8888/admin/addGift', this.formData)
        this.$refs.uploads.submit()
        if (res.meta.status !== 200) return this.$message.error('新增礼品信息失败')
        this.$message.success('新增礼品信息成功')
